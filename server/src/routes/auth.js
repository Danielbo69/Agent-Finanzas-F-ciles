...
module.exports = (prisma) => {
  ...
  // Register: accepts username, email, password, full name
  router.post('/register', async (req, res) => {
    const { username, email, password, name: fullName } = req.body;
    console.log('/auth/register body:', { username, email, name: fullName });
    if (!username || !email || !password) return res.status(400).json({ error: 'Missing fields' });
    const hashed = await bcrypt.hash(password, 10);
    try {
      const user = await prisma.user.create({ data: { email, username, password: hashed, name: fullName } });
      return res.json({ id: user.id, email: user.email, username: user.username, name: user.name });
    } catch (e) {
  ...
  // Login: accept username or email in the `username` field
  router.post('/login', async (req, res) => {
    const { username, password } = req.body;
    console.log('/auth/login body:', { username });
    if (!username || !password) return res.status(400).json({ error: 'Missing fields' });
    // Try to find by email or by username
    const user = await prisma.user.findFirst({ where: { OR: [{ email: username }, { username: username }] } });
